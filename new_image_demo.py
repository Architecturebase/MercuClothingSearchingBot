...
def detectioner(photo_path):
    ...
    if len(detections) != 0:
        ...
        for x1, y1, x2, y2, cls_conf, cls_pred in detections:
            ...
            cloth_type.append(classes[int(cls_pred)])
            ...
            img_id = photo_path.split('/')[-1].split('.')[0]
            cv2.imwrite('output/ouput-test_{}_{}_{}.jpg'.format(img_id, model, dataset), img)
            path = 'output/ouput-test_{}_{}_{}.jpg'.format(img_id, model, dataset)
            cropped_img = Image.open(path)  # new
            cropped_img = cropped_img.crop((x1, y1, x2, y2)).save('cropped/cropped_{}.jpg'.format(img_id))  # new
    else:
        print('Please, upload another photo')
        pass
    return cloth_type

# path = 'tg_img_loaded/losin.jpg'
# img = cv2.imread(path)
# detections = detectron.get_detections((img))
...
# cropped_img = Image.open(path) #new
# cropped_img = cropped_img.crop((x1, y1, x2, y2)).save('cropped/cropped_{}.jpg'.format(img_id)) #new
# #     cv2.imwrite('cropped/cropped_{}_{}_{}.jpg'.format(img_id,model,dataset),img)
# #     cv2.waitKey(0)